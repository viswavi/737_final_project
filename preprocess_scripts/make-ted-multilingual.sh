...
if [ $# -ge 2 ]; then
    ...
    if [ "$srclang" = "eng" ]; then
        ...
        target language via high-resource language. 
        Don't need to run it both ways!"
        exit 1
    fi
    if [ $# -eq 3 ]; then
      suffix=_$3
    ...
    echo 'Error: Must provide a source language code (e.g. aze or bel), and
     a high-resource language (e.g. tur or rus) to prepare data for
...
PROC_DDIR=data${suffix}/ted_processed/${srclang}${hrlang}_sepspm"$VOCAB_SIZE"/
BINARIZED_DDIR=fairseq/data-bin${suffix}/ted_${srclang}${hrlang}_sepspm"$VOCAB_SIZE"/
